...
import {
  KARE_NO_KIMOCHI_QUESTION_CHIPS,
  getKareNoKimochiReading,
...
function ResultSection({ eyebrow, title, paragraphs }: { eyebrow: string; title: string; paragraphs: string[] }) {
  return (
...
function ResultView({ result }: { result: KareNoKimochiReading }) {
  return (
    <div className="space-y-5">
        ...
        <div className="grid gap-0 lg:grid-cols-[220px_minmax(0,1fr)]">
          ...
          <div className="p-6 sm:p-7">
            <div className="flex flex-wrap items-center gap-2">
              <span className="rounded-full bg-[#f4ecdf] px-3 py-1 text-[11px] tracking-[0.12em] text-[#796a57]">FEELINGS TAROT</span>
              <span className="rounded-full bg-[#f1ebf8] px-3 py-1 text-[11px] text-[#756683]">{result.isReversed ? "逆位置" : "正位置"}</span>
            </div>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-3">
              <SummaryChip label="心の温度" value={result.heartTone} />
              <SummaryChip label="今の距離感" value={result.distanceLabel} />
              <SummaryChip label="動き方の鍵" value={result.guidanceLabel} />
            </div>
          </div>
        </div>
      </section>
        ...
        <aside className="rounded-[1.8rem] border border-[#e6d9c7]/90 bg-white/78 p-5 shadow-[0_20px_44px_-34px_rgba(111,90,68,0.2)] sm:p-6">
          ...
          <div className="mt-5 space-y-3">
            <div className="rounded-[1.2rem] border border-[#eee2cf] bg-[#fffdfa] px-4 py-4">
              <p className="text-[11px] tracking-[0.14em] text-[#8a7b68]">見えている本音</p>
              <p className="mt-2 text-sm leading-7 text-[#4f473d]">{result.partnerEmotion}</p>
            </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <ResultSection eyebrow="CARD" title="カードの解釈" paragraphs={splitParagraphs(result.cardInterpretation)} />
...
export default function KareNoKimochiClient() {
  ...
  const templateResult = useMemo(() => {
    ...
    try {
      return getKareNoKimochiReading(submittedQuestion);
    } catch {
      return null;
    }
  }, [submittedQuestion]);

  const { reading: result, isEnhancing } = useClaudeReading<KareNoKimochiReading>({
    feature: "kare-no-kimochi",
  ...
  return (
    <PageShell
      maxWidth="content"
      title="あの人の気持ち占い"
      description="あの人の本音、今の気持ち、そしてこれからの関係の流れを、ルミナの導きとタロットで静かに読み解きます。"
      backHref="/"
      backLabel="トップへ戻る"
    ...
    >
      <div className="mb-5 overflow-hidden rounded-[2rem] border border-[#e6dac8]/80 bg-[linear-gradient(140deg,rgba(255,251,246,0.82),rgba(248,241,232,0.7))] shadow-[0_26px_56px_-40px_rgba(104,86,66,0.28)]">
        <Image src="/gazou/kimoti.png" alt="あの人の気持ち占いのイメージ" width={1050} height={500} className="w-full" priority />
      </div>
      <div className="mb-5 overflow-hidden rounded-[2rem] border border-[#e6dac8]/80 bg-[linear-gradient(140deg,rgba(255,251,246,0.9),rgba(248,241,232,0.82))] shadow-[0_26px_56px_-40px_rgba(104,86,66,0.3)]">
        <div className="relative px-6 py-7 sm:px-8 sm:py-9">
          ...
          <div className="relative">
            <p className="text-xs tracking-[0.22em] text-[#8b7e6c]">KARE NO KIMOCHI TAROT</p>
            <h2 className="mt-3 text-[1.9rem] leading-tight text-[#2f2a27] sm:text-[2.35rem]">あの人の心にあるものを、<br />静かに見つめるために。</h2>
            <div className="mt-5 max-w-[38rem] space-y-4 text-[0.98rem] leading-8 text-[#544c42]">
              <p>言葉にされない気持ちほど、胸の中で大きくなりやすいものです。</p>
              <p>このページでは、相手の本音や感情の動き、これからの関係の流れを読み解いていきます。</p>
            </div>
          </div>
        </div>
      </div>
      <GlassCard className="rounded-[2rem] p-5 sm:p-6">
        <div className="rounded-[1.7rem] border border-[#eadfce]/80 bg-[linear-gradient(180deg,rgba(255,252,248,0.96),rgba(250,244,235,0.85))] p-5 shadow-[0_22px_44px_-34px_rgba(112,90,67,0.2)]">
          ...
          <p className="mt-3 text-base leading-8 text-[#4f473d]">あの人の本音、今の感情、あなたへの距離感、これからの関係の流れをやさしく読み解きます。</p>
        </div>
        <form onSubmit={handleSubmit} className="mt-5">
          <label htmlFor="kare-no-kimochi-question" className="block text-sm font-medium text-[#3b352f]">あの人に聞きたいこと</label>
      ...
      <GlassCard className="mt-5 rounded-[2rem] p-5 sm:p-6">
        {result ? (
          ...
          <div className="flex min-h-[320px] flex-col justify-center rounded-[1.8rem] border border-dashed border-[#e4d9c7] bg-[linear-gradient(180deg,rgba(255,252,248,0.82),rgba(249,244,236,0.68))] px-6 py-10 text-center">
            ...
            <h2 className="mt-3 text-[1.4rem] text-[#2f2a27]">カードが相手の気持ちを映し出します</h2>
            <p className="mt-4 text-sm leading-7 text-[#5c5448]">質問を書いてカードを開くと、<br />あの人の本音や関係の流れを表示します。</p>
          </div>
        )}
      </GlassCard>
    </PageShell>
