...
export function WallpapersClient() {
  ...
  return (
    <GlassCard>
      <h2 className="text-lg font-medium text-[#2e2a26]">Monthly Unlock Reward</h2>
      <p className="mt-2 text-sm leading-relaxed text-[#544c42]">
        Check in daily. Once you reach 7 visit days this month, you can unlock and download the wallpaper.
      </p>
      ...
      <div className="mt-4 rounded-2xl border border-[#e1d5bf]/75 bg-[linear-gradient(162deg,rgba(255,252,246,0.9),rgba(248,242,231,0.86))] p-4">
        <p className="text-xs tracking-[0.08em] text-[#847967]">Progress</p>
        <p className="mt-1 text-sm text-[#544c42]">{Math.min(monthlyCount, 7)}/7 days</p>
        {unlocked ? <p className="mt-2 text-sm text-[#5f6b52]">Unlocked</p> : null}
        {!unlocked && monthlyCount < 7 ? <p className="mt-2 text-sm text-[#6f6556]">{7 - monthlyCount} day(s) remaining</p> : null}
        {!unlocked ? (
          <div className="mt-3">
        ...
        <div className={`relative mx-auto max-w-[260px] overflow-hidden rounded-2xl border border-[#dfd2bc]/78 ${unlocked ? "" : "pointer-events-none"}`}>
          ...
          {!unlocked ? (
            <div className="absolute inset-0 flex items-center justify-center bg-[#f9f2e5]/35">
              <p className="rounded-full border border-[#d3c4a7]/80 bg-[#fffaf0]/90 px-3 py-1 text-xs text-[#6f6556]">Unlock at 7 days</p>
            </div>
          ) : null}
        </div>
