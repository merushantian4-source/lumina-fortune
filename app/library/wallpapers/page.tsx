...
export default function MonthlyWallpaperPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (searchParams.get("claim") === "1" && current && current.monthlyVisitCount >= 7 && !current.monthlyClaimed) {
        const claimed = claimMonthlyGiftForVisitor(localStorage, key);
        timeoutId = window.setTimeout(() => {
          setRecord(claimed);
          setNotice("今月の贈り物を受け取りました。");
        }, 0);
      }
    } catch {
      timeoutId = window.setTimeout(() => {
  ...
  return (
    <PageShell
      maxWidth="content"
      title="白の館の贈り物"
      description="毎日の訪問記録に応じて、今月限定の待ち受けを受け取れます。"
      backHref="/"
      backLabel="トップへ戻る"
    >
      <GlassCard>
        <h2 className="text-lg font-medium text-[#2e2a26]">今月の限定待ち受け</h2>
        <p className="mt-2 text-sm leading-relaxed text-[#544c42]">
          白の館から届いた、今月の贈り物です。7日達成で受け取りが開放されます。
        </p>
        ...
        <div className="mt-4 rounded-2xl border border-[#e1d5bf]/75 bg-[linear-gradient(162deg,rgba(255,252,246,0.9),rgba(248,242,231,0.86))] p-4">
          <p className="text-xs tracking-[0.08em] text-[#847967]">達成状況</p>
          ...
          {!unlocked && monthlyCount < 7 ? <p className="mt-2 text-sm text-[#6f6556]">あと{7 - monthlyCount}日で受け取れます</p> : null}
          {!unlocked ? (
            <div className="mt-3">
          ...
          <div className={`relative mx-auto max-w-[260px] overflow-hidden rounded-2xl border border-[#dfd2bc]/78 ${unlocked ? "" : "pointer-events-none"}`}>
            ...
            {!unlocked ? (
              <div className="absolute inset-0 flex items-center justify-center bg-[#f9f2e5]/35">
                <p className="rounded-full border border-[#d3c4a7]/80 bg-[#fffaf0]/90 px-3 py-1 text-xs text-[#6f6556]">7日達成で開放</p>
              </div>
            ) : null}
          </div>
